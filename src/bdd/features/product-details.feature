...
Feature: Product Details
  ...
  Scenario: Product title matches the selected product from the list
    ...
    And discounted and original price should be consistent if present
  ...
  # 6. Buy Now
  Scenario: Buy now redirects to checkout (or cart based on app flow)
    When I open product details for "phone" from search results
    And I click buy now on product details
    Then I should be redirected to checkout or cart

  # 7. Wishlist / Favorite
  Scenario: Wishlist action either saves or requires login
    When I open product details for "phone" from search results
    And I click wishlist on product details
    Then wishlist should be updated or login should be required

  # 8. Product Description Section
  Scenario: Description section expands with See More (if available)
    When I open product details for "phone" from search results
    And I expand the product description if available
    Then product description content should be visible

  # 9. Tabs Functionality (Overview / Reviews)
  Scenario: Switching tabs loads content (if tabs exist)
    When I open product details for "phone" from search results
  ...
  Scenario: Refresh keeps the same product and back returns to previous page
    When I open product details for "phone" from search results
  ...
  # 15. Error Handling
  Scenario: Invalid product URL shows error or 404 page
